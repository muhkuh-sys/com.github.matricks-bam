...
for strMatch in glob.iglob(os.path.join(strCwd, 'bam-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'bam-*.xml'))
if len(astrMatch) == 0:
    raise Exception('No match found for "bam-*.xml".')
elif len(astrMatch) > 1:
    raise Exception('More than one match found for "bam-*.xml". Clean your build folder.')
